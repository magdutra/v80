...
DWORD CJV3::Load(HANDLE hFile, DWORD dwFlags)
{
    ...
    if (ReadFile(hFile, &m_pHeader[0], sizeof(JV3_HEADER), &dwBytes, NULL) == 0)
    {
        dwError = GetLastError();
        goto Done;
    }
    ...
    if (GetFileSize(hFile, NULL) > (dwBytes + sizeof(JV3_HEADER)))
    {

        // Position file pointer at the second header
        if (SetFilePointer(hFile, dwBytes, NULL, FILE_BEGIN) == 0xFFFFFFFF)
            throw GetLastError();

        // Read second header to m_pHeader[1]
        if (ReadFile(hFile, &m_pHeader[1], sizeof(JV3_HEADER), &dwBytes, NULL) == 0)
            throw GetLastError();
...
DWORD CJV3::Read(BYTE nTrack, BYTE nSide, BYTE nSector, BYTE* pBuffer, WORD wSize)
{
    ...
    if (ReadFile(m_hFile, pBuffer, pTrack->wSectorSize, &dwBytes, NULL) == 0)
    {
        dwError = GetLastError();
        goto Done;
    }
