...
CDMK::~CDMK()
{
    ...
    if (m_pTrack != NULL)
        HeapFree(GetProcessHeap(), 0, m_pTrack);
...
DWORD CDMK::Load(HANDLE hFile, DWORD dwFlags)
{
    ...
    // Read disk header
    if (ReadFile(hFile, &m_Header, sizeof(m_Header), &dwBytes, NULL) == 0)
    {
        dwError = GetLastError();
        goto Done;
    }
    ...
    if ((m_pTrack = (BYTE*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwBytes)) == NULL)
    {
...
DWORD CDMK::FindGeometry()
{
    ...
    for (int t = 0; t < 4; t++)
    {
        ...
        if (SetFilePointer(m_hFile, dwTrack[t], NULL, FILE_BEGIN) == 0xFFFFFFFF)
        {
            dwError = GetLastError();
            goto Done;
        }
        ...
        // Read track header
        if (ReadFile(m_hFile, m_pTrack, m_Header.wTrackLength, &dwBytes, NULL) == 0)
        {
            dwError = GetLastError();
            goto Done;
        }
...
DWORD CDMK::LoadTrack(BYTE nTrack, BYTE nSide)
{
    ...
    if (nTrack != m_nCacheTrack || nSide != m_nCacheSide)
    {
        ...
        // Set file pointer
        if (SetFilePointer(m_hFile, dwBytes, NULL, FILE_BEGIN) == 0xFFFFFFFF)
        {
            dwError = GetLastError();
            goto Done;
        }
...
DWORD CDMK::SaveTrack()
{
    ...
    if (m_bCacheWrite)
    {
        ...
        // Write track
        if  (WriteFile(m_hFile, m_pTrack, m_Header.wTrackLength, &dwBytes, NULL) == 0)
        {
            dwError = GetLastError();
            goto Done;
        }
